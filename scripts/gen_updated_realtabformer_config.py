...
src_config = EXP_DIR / "california" / "realtabformer" / "config.toml"

conf = src_config.read_text()

for data_path in EXP_DIR.glob("*"):
    if data_path.is_file() or data_path.name == "california":
        continue
    ...
    cconf = str(conf)
    cconf = cconf.replace("/california/", f"/{data_path.name}/")
